...
impl PipelineDirection {
    pub fn axis(&self) -> Axis {
        match self {
            PipelineDirection::Up | PipelineDirection::Down => Axis::Vertical,
            PipelineDirection::Left | PipelineDirection::Right => Axis::Horizontal,
        }
    }

    pub fn is_vertical(&self) -> bool {
        matches!(self, PipelineDirection::Up | PipelineDirection::Down)
    }

    pub fn is_horizontal(&self) -> bool {
        matches!(self, PipelineDirection::Left | PipelineDirection::Right)
    }
}
...
impl Styled for PipelineLayout {
...
#[allow(unused)]
impl PipelineLayout {
    pub fn direction(mut self, direction: PipelineDirection) -> Self {
...
pub struct PipelineLayoutFrameState {
...
impl IntoElement for PipelineLayout {
    type Element = Self;
...
impl InteractiveElement for PipelineLayout {
    fn interactivity(&mut self) -> &mut gpui::Interactivity {
        self.base.interactivity()
    }
}

impl Element for PipelineLayout {
    type RequestLayoutState = PipelineLayoutFrameState;
    ...
    fn request_layout(
        ...
        let gap = style
            .gap
            .along(self.direction.axis())
            .to_pixels(font_size.into(), rem_size);
        let item_size_with_gap = self.item_size.along(self.direction.axis()) + gap;
        let longest_item_size = self.measure_item(None, window, cx);

        let content_size = if self.direction.axis().is_horizontal() {
            Size {
