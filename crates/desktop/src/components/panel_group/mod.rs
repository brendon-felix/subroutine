...
impl SidePanel {
    ...
    pub fn initial_width(mut self, width: Pixels) -> Self {
...
pub struct SidePanelState {
    /// The proportion of the total group width occupied by this side panel when open.
    /// Clamped to `proportion_range` during resizing.
    pub proportion_range: Range<f32>,
...
impl PanelGroupState {
    ...
    fn resize_left(&mut self, mouse_x: Pixels) {
        ...
        let clamped = new_proportion
            .max(panel.proportion_range.start)
            .min(panel.proportion_range.end.min(max));
    ...
    fn resize_right(&mut self, mouse_x: Pixels) {
        ...
        let left_proportion = self
            ...
            .map(|p| p.opened_proportion)
            .unwrap_or(0.0);
        let max = (1.0 - self.center_panel.min_proportion - left_proportion)
            .max(panel.proportion_range.start);
        let clamped = new_proportion
            .max(panel.proportion_range.start)
            .min(panel.proportion_range.end.min(max));
