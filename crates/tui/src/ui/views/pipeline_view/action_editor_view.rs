...
pub enum EditorOutcome {
    ...
    /// Return to the list.
    Back,
    /// Return to the list and complete the action with this id.
    Complete(Uuid),
    /// Return to the list and delete the item with this id.
    Delete(Uuid),
}

pub struct ActionEditorView {
    action: Action,
}
...
impl ActionEditorView {
    ...
    pub fn handle_key(&mut self, key: KeyEvent) -> EditorOutcome {
        match key.code {
            ...
            KeyCode::Char('c') => EditorOutcome::Complete(self.action.id),
            KeyCode::Char('d') => EditorOutcome::Delete(self.action.id),
            _ => EditorOutcome::Stay,
        }
    }
}

impl AppView for ActionEditorView {
    ...
    fn draw(&mut self, f: &mut Frame, area: Rect, _last_frame: Instant) {
        ...
        // ── Duration ──────────────────────────────────────────────────────
        let dur_val = match action.duration {
            Some(d) => {
                let m = d.num_minutes();
                if m >= 60 {
