...
pub(super) const MIN_ITEM_WIDTH: Pixels = px(32. * 4.);
pub(super) const MIN_ITEM_HEIGHT: Pixels = px(16. * 4.);
// pub(super) const BASE_ITEM_WIDTH: Pixels = px(72. * 4.);
...
impl TimelineItem {
    ...
    pub(super) fn colors(&self, cx: &App) -> ButtonColors {
        // match self.item {
        //     AnyItem::Action(_) => ButtonColors::outline(cx.theme().button_primary, cx),
        ...
        //         cx,
        //     ),
        //     AnyItem::Routine(_) => ButtonColors::outline(cx.theme().foreground, cx),
        // }
        let bg = cx.theme().group_box;
        match self.item {
            AnyItem::Action(_) => ButtonColors::solid(cx.theme().button_primary, bg, cx),
            AnyItem::Event(_) => ButtonColors::solid(
                cx.theme()
                    .button_primary
                    .mix_oklab(cx.theme().foreground, 0.5),
                bg,
            ...
            AnyItem::Routine(_) => ButtonColors::solid(cx.theme().foreground, bg, cx),
        }
    }
}
...
impl TitleEditState {
    pub(super) fn new(initial: String) -> Self {
        let draft = parse_action(&initial).ok();
        Self {
    ...
    pub(super) fn update_action(&mut self, text: String) {
        ...
        match parse_action(&self.current_text) {
            Ok(d) => {
                self.draft = Some(d);
    ...
    pub(super) fn update_event(&mut self, text: String) {
        self.current_text = text.trim().to_string();
        match parse_event(&self.current_text) {
            Ok(d) => {
                self.draft = Some(d);
                self.parse_error = false;
            }
...
pub(super) fn render_item_preview(
    ...
    use crate::utils::ButtonColorizeExt;
    use gpui_component::h_flex;
    use gpui_squircle::SquircleStyled;
    let too_short = false;
    div()
        .w(item_w)
        .h(item_h)
